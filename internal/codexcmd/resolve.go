...
// The bool is false when the command is not a Codex command or the platform is
// not Windows, allowing callers to retain their normal shell behavior.
...
func resolveWindowsExecutable(localAppData string, userProfile string, lookPath func(string) (string, error)) (string, error) {
	...
	if localAppData != "" {
		...
		for _, entry := range entries {
			if entry.IsDir() {
				// Versioned app binaries are the real executable. Prefer them over
				// launcher/proxy copies, which can hand off to a child process and
				// make cancellation leak that child.
